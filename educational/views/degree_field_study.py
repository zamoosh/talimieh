...
def degree_field_study(request):
    context = {}
    context['degree_field_studys'] = Degree_field_study.objects.filter(status=True).order_by('-id')
    if request.method == "POST":
        context['req'] = {}
        context['req']['title'] = request.POST.get('title', '').strip()
        context['req']['doc'] = request.POST.get('doc', '').strip()
        degree_field_study = Degree_field_study()
    ...
    if 'remove' in request.GET:
        context['rmYear'] = Degree_field_study.objects.get(id=int(request.GET.get('remove')))
